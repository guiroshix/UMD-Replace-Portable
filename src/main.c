...
void Title(void) {
  printf(
    "\n"
    "UMD-REPLACE version %s - Copyright (C) 2012-2015 CUE | Compiled by Guiroshix / BomberME\n"
    "Tiny tool to replace data files in a PSP UMD ISO\n"
    "\n",
    VERSION
  );
...
/*----------------------------------------------------------------------------*/
void Replace(char *isoname, char *oldname, char *newname) {
  char oldname_buffer[256];

  snprintf(oldname_buffer, sizeof(oldname_buffer), "%s", oldname);
  oldname = oldname_buffer;
...
void TOC(char *isoname, int lba, int len, U64 found, int lba_old, int diff) {
  ...
  for (i = 0; i < total; i++) {
    ...
    while (pos < len) {
      ...
      // update needed?
      if ((newlba > lba_old) || ((newlba == lba_old) &&
